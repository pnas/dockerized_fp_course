...
distinctF =
  let 
    insertAndCheckNotMembershipGT100Abort elm =
      StateT
        (\uniques ->
          if elm > 100
            then
  ...
  in
  (`evalT` S.empty)
  . filtering insertAndCheckNotMembershipGT100Abort
...
distinctG =
  runOptionalT
  . (`evalT` S.empty)
  . filtering insertAndCheckNotMemebrshipAbortForGT100
  where
    loggerGT100 elm = 
      log1
        (listh $ "aborting > 100: " P.++ show elm)
        Empty
    loggerEven elm uniques =
      log1
        (listh $ "even number: " P.++ show elm)
        $ Full (S.notMember elm uniques, S.insert elm uniques)
    loggerOdd elm uniques =
      Logger
        Nil
        $ Full (S.notMember elm uniques, S.insert elm uniques)

    insertAndCheckNotMemebrshipAbortForGT100 elm
      | elm > 100        = StateT $ const (OptionalT $ loggerGT100 elm)
      | elm `mod` 2 == 0 = StateT $ OptionalT . loggerEven elm
      | otherwise        = StateT $ OptionalT . loggerOdd elm
